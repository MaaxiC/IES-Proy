...
const ItemDetail = () => {
  ...
  const plusItem = () => {
    if (data.stock > (quantity + qty)) setQty(qty + 1)
  }

  const addToCart = () => {
    if (data.stock >= (quantity + qty)) {
        ...
        >
            <div className="details">
                ...
                <div className="box">
                    ...
                    {data.stock > (quantity + qty) ? <p>Stock restante: {data.stock - (quantity + qty)}</p> : <p style={{ color: "red" }}>Sin Stock</p>}
                    ...
                    {data.stock > (quantity + qty - 1) ? <Button variant="info" className="w-100" onClick={addToCart}>Agregar al carrito</Button> : <Button variant="info" className="w-100" disabled>Agregar al carrito</Button>}
                </div>
            </div>
        </Card>
    </Container>
